...
class dataset():
    ...
    def __comment_similarity(self, tweet_id, no_similarities):
        conn = sqlite3.connect('bamman_dataset.db')
        c = conn.cursor()
        orig_tweet = list(c.execute("SELECT tweet_text FROM labeled_tweets WHERE id='%s'" % tweet_id))[0][0]
        c = conn.cursor()
        tweets = c.execute("SELECT tweet_text FROM author_tweets WHERE orig_tweet_id='%s' LIMIT %d" % (tweet_id, no_similarities))
        ...
        if len(tweets) == 0:
            ...
            if len(hashtags) != 0:
                ...
                for hashtag in hashtags:
                    if query == "":
                        query = "SELECT tweet_text FROM author_tweets WHERE tweet_text LIKE '\%" + hashtag +"\%'"
                    else:
