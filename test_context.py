...
similarities = wallace_dataset.dataset().compute_comments_similarities(no_similarities)
sim = np.array(similarities)
sim.resize(X.shape[0], no_similarities)
np.hstack( (X.toarray(), np.array(sim) ) )
...
tweets, targets = get_data('riloff', affective_norms=affective_norms, punctuation=punctuation, emoticons=emoticons)

vectorizer = TfidfVectorizer(ngram_range=(1,2), binary=True, stop_words="english", max_features=max_features)
X = vectorizer.fit_transform(tweets)

similarities = riloff_dataset.dataset().compute_comments_similarities(no_similarities)
sim = np.array(similarities)
sim.resize(X.shape[0], no_similarities)
np.hstack( (X.toarray(), np.array(sim) ) )
